...
pacman::p_load(haven, dplyr, sjPlot, DiagrammeR, DiagrammeRsvg, rsvg, table1, flextable, 
               systemfonts, VIM, mice, lattice, odds.n.ends, ggplot2, broom, blorr, 
...
df_race_income <- df %>% select(race, income_ratio)
marginplot(df %>% dplyr::select(race, income_ratio), col=c("blue","red","orange"), cex=1, cex.lab=1, cex.numbers=0.7, pch=20) 
...
model_1 <- with(imp,glm(first_molar_presence ~ age + gender + race + obesity + income_ratio, family = 'binomial'))
